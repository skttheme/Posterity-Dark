...
if ( ! function_exists( 'posterity_get_header_toolbar' ) ) {
	...
	function posterity_get_header_toolbar( &$icons ) {
		...
		//prepare icons HTML
		$tools_html =
		        ( $basket_sidebar ? '<button id="basket-menu-switch" class="'.esc_attr($basket_sidebar_icon).' tool" title="' . esc_attr__( 'Shop sidebar', 'posterity-dark' ) . '"><span id="basket-items-count" class="zero">' . esc_html( $woocommerce->cart->cart_contents_count ) . '</span><span class="screen-reader-text">' . esc_attr__( 'Shop sidebar', 'posterity-dark' ) . '</span></button>' : '' ) .
		        ( $hidden_sidebar ? '<button id="side-menu-switch" class="'.esc_attr($hidden_sidebar_icon).' tool" title="' . esc_attr__( 'More info', 'posterity-dark' ) . '">'.($hs_type === 'animated'? '<i></i>' : '').'<span class="screen-reader-text">' . esc_attr__( 'More info', 'posterity-dark' ) . '</span></button>' : '' ) .
